...
import { readLong, readShort, sliceLongToBytes } from "https://deno.land/x/io/ioutil.ts";
import { Sha1 } from "https://deno.land/x/ws/sha1.ts";
import { writeResponse } from "https://deno.land/x/http/server.ts";
...
export async function readFrame(buf: BufReader): Promise<WebSocketFrame> {
  ...
  console.log("signature: ", b ); // 268435455
  switch (b >>> 4) {
    case 0b1000:
      isLastFrame = true;
      break;
...
export async function* receiveFrame(
  ...
  console.log("receiveFrame,", receiving);
...
class WebSocketImpl implements WebSocket {
  ...
  async *receive(): AsyncIterableIterator<WebSocketEvent> {
    ...
    console.log("*receive, ", payloadsLength);
